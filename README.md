...
已核验（`verified`）排放样本不允许普通字段更新，只能通过 `POST /api/samples/:id/revisions` 追加修订。

| 场景 | 结果 |
|---|---|
| 填写修订原因并提交 | 保留原值基线快照（首次修订时自动回填），追加下一条修订，版本号 +1 |
| 修订原因为空 | 整次拒绝（422），不更新字段、不新增修订 |
| `expectedVersion` 过期 | 整次拒绝（409），不更新字段、不新增修订 |
| 同一编码并发提交 | 事务行锁串行化，仅一条成功，其余按版本冲突整次拒绝 |
| 对 verified 样本调用普通 PUT | 整次拒绝（422），必须走修订接口 |
| 对非 verified 样本调用修订接口 | 整次拒绝（422），仍走原有更新/状态迁移 |

- 修订历史只追加、不可覆盖；响应中 `current=true` 的最高版本修订是唯一可作为关联合规判断依据的版本。
- 样本页展示当前修订号、逐版本历史差异（旧值 → 新值）、修订人/请求 ID，以及最近一次拒绝原因；拒绝原因在页面刷新后仍可读到，成功修订后清除。
- 每次成功修订写入 `revise` 审计（版本前后值、原因、actor、request ID）；被拒绝的请求不写审计。状态迁移图、RBAC 边界（写操作 operator+，删除 admin，审计 reviewer+）与既有审计字段保持不变。
