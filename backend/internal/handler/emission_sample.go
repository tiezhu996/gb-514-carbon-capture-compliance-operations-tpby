...
type EmissionSampleHandler struct{ service service.EmissionSampleService }

func NewEmissionSampleHandler(s service.EmissionSampleService) *EmissionSampleHandler {
	return &EmissionSampleHandler{service: s}
}

func (h *EmissionSampleHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/samples")
	resource.GET("", h.list)
...
func (h *EmissionSampleHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *EmissionSampleHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *EmissionSampleHandler) create(c *gin.Context) {
	var input dto.CreateEmissionSample
...
func (h *EmissionSampleHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateEmissionSample
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *EmissionSampleHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *EmissionSampleHandler) remove(c *gin.Context) {
