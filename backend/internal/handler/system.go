...
type SystemHandler struct {
	...
	captureUnit        service.CaptureUnitService
	permitRule         service.PermitRuleService
	emissionSample     service.EmissionSampleService
	complianceDecision service.ComplianceDecisionService
	db                 *gorm.DB
	redis              *redis.Client
}

func NewSystemHandler(security service.SecurityService, captureUnit service.CaptureUnitService, permitRule service.PermitRuleService, emissionSample service.EmissionSampleService, complianceDecision service.ComplianceDecisionService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, captureUnit: captureUnit, permitRule: permitRule, emissionSample: emissionSample, complianceDecision: complianceDecision, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	captureUnitCounts, err := h.captureUnit.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["units"] = captureUnitCounts

	permitRuleCounts, err := h.permitRule.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["rules"] = permitRuleCounts

	emissionSampleCounts, err := h.emissionSample.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["samples"] = emissionSampleCounts

	complianceDecisionCounts, err := h.complianceDecision.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["decisions"] = complianceDecisionCounts
